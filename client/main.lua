		...
		for k,doorID in ipairs(Config.DoorList) do
			...
			if distance < maxDistance then
				...
				RegisterCommand('door', function(source, args, rawCommand)
					...
					ESX.TriggerServerCallback('esx_doorlock:openDoor', function(inventory)
				...
				RegisterCommand('doorlk', function(source, args, rawCommand)

					if doorID.locked == false then
						print('nao funciona')

					else
                    ESX.TriggerServerCallback('esx_doorlock:lockpickDoor', function(inventory)
                        local item = inventory.items
                        if item.count >= 1 then
                    
							local prb = math.random(1, 500)
							exports['progressBars']:startUI(15000, "Lockpicking...")
					RequestAnimDict("mini@safe_cracking")
					TaskPlayAnim(GetPlayerPed(-1),"mini@safe_cracking", "idle_base", 1.0,-1.0, 15000, 1, 1, true, true, true)
					Citizen.Wait(15000)
                
                            if prb >= 60 and prb <= 110 then
                                doorID.locked = not doorID.locked
                            else
								print('Lockpick fracassado')
